...
public class CoustomerBillService {
	...
	public ResponseEntity<ResponseDTO<Map<String, Object>>> getAllBills(Integer pageNum, Integer pageSize,
		...
		Page<CoustomerBill> billPage = coustomerBillRepository.findByCompanyId(companyId, pageable);
	...
	public void generateAndSendBills() {
		...
		// List<CoustomerBill> bills = coustomerBillRepository.findAll();// find by
	...
	public ResponseEntity<ResponseDTO<Map<String, Object>>> getFilteredBills(Integer pageNum, Integer pageSize,
			String email, String phno, String status, Long companyId) {
		Pageable pageable = PageRequest.of(Math.max(0, pageNum - 1), pageSize, Sort.by(Sort.Order.desc("billingDate")));
		...
		for (CoustomerBill bill : dueBillsBeforeTwoDays) {
			...
			if (client != null) {
				...
				Map<String, ?> notificationRequest = Map.of(
						...
						Constants.FIELD_NOTIFICATION_TEXT,
						"Your bill is due in 2 days. Please make the payment to avoid late fees.");
